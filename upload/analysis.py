...
if files:
    ...
    if st.button("Load Document"):
        ...
        if os.path.exists(img_file):
            if os.path.exists(rects_file):
                st.session_state['selected_file'] = img_file
                st.success(f"Selected file: {selected_file}")
            else:
                st.error(f"JSON file not found for the selected image: {rects_file}")
        else:
            st.error(f"Image file not found: {img_file}")

def run(selected_file, rects_file, labels):
    ui_width = st_js.st_javascript("window.innerWidth")

    try:
        docImg = Image.open(selected_file)
